...
public class CustomerQueryHandler : 
    ...
    IRequestHandler<GetCustomersByIncludesQuery, ApiResponse<List<CustomerResponse>>>,
    IRequestHandler<GetCustomersByNameQuery, ApiResponse<List<CustomerResponse>>>
{
    ...
    public async Task<ApiResponse<List<CustomerResponse>>> Handle(GetCustomersByIncludesQuery request, CancellationToken cancellationToken)
    {
        List<Customer> entityList = await unitOfWork.CustomerRepository.GetAllIncluding(x => x.CustomerAddresses, x => x.CustomerPhones);
        var mappedList = mapper.Map<List<CustomerResponse>>(entityList);
        return new ApiResponse<List<CustomerResponse>>(mappedList);
    }
    ...
    {
        List<Customer> entityList = await unitOfWork.CustomerRepository.Find(x => x.FirstName == request.Name);
        var mappedList = mapper.Map<List<CustomerResponse>>(entityList);
        return new ApiResponse<List<CustomerResponse>>(mappedList);
    }
}